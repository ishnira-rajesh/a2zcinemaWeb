...
.controller('UploadControllerGif',['$scope','$window','$localStorage','$http', function($scope, $window, $localStorage, $http) {
  ...
  $scope.upload = function() {
   ...
   // alert('image controller Flag1' + $scope.file);
    if($scope.file) {
        ...
        var uniqueFileName = $scope.imageNames.gifImageName = $scope.uniqueString() + '-' + $scope.file.name;
       ...
       // alert('image controller file name'+ $scope.imageName );
        ...
        bucket.putObject(params, function(err, data) {
          if(err) {
        	...
        	$http.post("http://localhost:8080/A2zcinema/uploadGifImage/",JSON.stringify($scope.imageNames))
						.then(function(response) {
						 ...
						 console.error('Error while save latest gif.');
			});
        	
        	
        	
            // Upload Successfully Finished
            toastr.success('Gif Image Uploaded Successfully.');
		...
		[
				...
				function($scope, $window, $localStorage, $http) {
					...
					/*
					  ...
					  * */
					     self.imagedata = {};
					  	  $http.get("http://localhost:8080/A2zcinema/getGifImage/")
					  		   .then(function(response) {
					  			   self.imagedata = response.data;
					  			  /// console.log(' user image' , self.imagedata.image_name);
					  		   }, function(errResponse) {
					  			 console.log('Error while fetching GifImage');
					  		    //alert("Error while fetching gif.")
					          });
					   
					  	$scope.rowDelete = function(gif_image_id){
					    	   $http.get("http://a2zcinemaworld.com/deleteGifImage/"+gif_image_id)
							   .then(function(response) {
