...
.controller('UploadControllerVedio',['$scope','$window','$localStorage','$http', function($scope, $window, $localStorage, $http) {
  ...
  $scope.upload = function() {
   ...
   // alert('image controller Flag1' + $scope.file);
    if($scope.file) {
        ...
        var uniqueFileName = $scope.imageNames.movieTrailerImageName = $scope.uniqueString() + '-' + $scope.file.name;
       ...
       // alert('image controller file name'+ $scope.imageName );
        ...
        bucket.putObject(params, function(err, data) {
          if(err) {
        	...
        	$http.post("http://localhost:8080/A2zcinema/uploadMovieTrailerImage/",JSON.stringify($scope.imageNames))
            ...
            toastr.success('Movie Trailer Image Uploaded Successfully.');
...
angular.module('profileApp').filter("trustUrl", ['$sce', function ($sce) {
					...
					/*
					  ...
					  * */
					     self.imagedata = {};
					  	...
					  	$scope.rowDelete = function(movie_trailer_image_id){
					    	   $http.get("http://localhost:8080/A2zcinema/deleteMovieTrailerImage/"+movie_trailer_image_id)
							   .then(function(response) {
