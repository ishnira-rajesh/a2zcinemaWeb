  ...
  $scope.upload = function() {
   ...
   // alert('image controller Flag1' + $scope.file);
    if($scope.file) {
        ...
        var uniqueFileName = $scope.imageNames.imageName = $scope.uniqueString() + '-' + $scope.file.name;
       ...
       // alert('image controller file name'+ $scope.imageName );
        ...
        bucket.putObject(params, function(err, data) {
          if(err) {
        	...
        	$http.post("http://localhost:8080/A2zcinema/uploadMultipleFile/",JSON.stringify($scope.imageNames))
		...
		[
				...
				function($scope, $window, $localStorage, $http) {
					...
					/*
					  ...
					  * */
					     self.imagedata = {};
					  	  $http.get("http://localhost:8080/A2zcinema/getProfileImageByUserId/"+$window.localStorage.getItem('userId'))
					  		   .then(function(response) {
					  	...
					  	$scope.rowDelete = function(profile_image_id){
					    	   $http.get("http://localhost:8080/A2zcinema/deleteProfileImageByUserId/"+profile_image_id)
							   .then(function(response) {
